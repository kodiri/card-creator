  ...
  turnOn = (i) => {
    ...
    render() {
      return (
        <div className='tools'>
          <Tool
            title="BgColors" enabled={this.isOn(0)} f={() => { this.turnOn(0)}} name="Background Color" icon="eye-dropper" openProp={false} bgcolors={['skyblue', 'lightseagreen','lemonchiffon', 'violet', 'lightpink']} changeProperty={(key, value) => 
              this.props.changeProperty(key, value)}
          />
          <Tool
          title="Fonts" name="Font Style" enabled={this.isOn(1)} f={() => { this.turnOn(1)}} icon="text-width" 
        ...
        {/* Pdf doesn't accept "px" units */}
          <Tool
            title="Frames" name='Frames' icon="border-style" frames={['5px solid blue', '5px double yellow ', '5px outset purple']} 
            enabled={this.isOn(2)} f={() => { this.turnOn(2)}} 
            changeProperty={(key, value) => this.props.changeProperty(key, value)}
          />
          <Tool
            title="Colors" name="Text Color"  icon="paint-brush" enabled={this.isOn(3)} f={() => { this.turnOn(3)}} 
            colors={['skyblue', 'lightseagreen','lemonchiffon', 'violet', 'lightpink']} 
            changeProperty={(key, value) => this.props.changeProperty(key, value)}
          />
          <Tool
            title="BgImages" name="Background Image" icon="file-image" bgImages={['https://cdn.pixabay.com/photo/2017/08/12/10/13/background-2633962_960_720.jpg', 
